...
| `AssetToken.burn(address, uint256)` | Burns receipt tokens on redeem | `i_thunderLoan` only |
| `AssetToken.transferUnderlyingTo(address, uint256)` | Moves underlying ERC20 out of AssetToken | `i_thunderLoan` only |
| `AssetToken.updateExchangeRate(uint256 fee)` | Bumps the exchange rate upward | `i_thunderLoan` only |
